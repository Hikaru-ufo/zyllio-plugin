...
console.log('🚀 Chargement plugin Numéro de Semaine...');
...
class WeekNumberFunction {
    execute(properties) {
        ...
        try {
            ...
            // Essayer avec le service dictionary de Zyllio
            let dateValue = dateProperty.value;
            
            if (typeof zySdk !== 'undefined' && zySdk.services && zySdk.services.dictionary) {
                const valueFromDict = zySdk.services.dictionary.getValue(dateProperty);
                console.log('Valeur via dictionary:', valueFromDict);
                if (valueFromDict) {
